...
struct event {
    std::vector <double> p_B0, p_Dst, p_Dst_D0, p_D0_pi, p_D0_K, p_Dst_pi, p_mu, p_nu;
    double B0_PE, Dst_PE, Dst_D0_PE, D0_pi_PE, D0_K_PE, Dst_pi_PE, mu_PE, nu_PE;
    double B0_PX, Dst_PX, Dst_D0_PX, D0_pi_PX, D0_K_PX, Dst_pi_PX, mu_PX, nu_PX;
    double B0_PY, Dst_PY, Dst_D0_PY, D0_pi_PY, D0_K_PY, Dst_pi_PY, mu_PY, nu_PY;
    double B0_PZ, Dst_PZ, Dst_D0_PZ, D0_pi_PZ, D0_K_PZ, Dst_pi_PZ, mu_PZ, nu_PZ;
    int B0_ID, Dst_ID, Dst_D0_ID, D0_pi_ID, D0_K_ID, Dst_pi_ID, mu_ID, nu_ID;
    //std::vector <double> p_B0_tag, p_Dst_tag, p_Dst_D0_tag, p_Dst_pi_tag, p_D0_K_tag, p_D0_pi_tag, p_mu_tag, p_nu_tag;
...
int main( int /*argc*/, char** /*argv*/ )
{
    ...
    //    std::string fname = "muola";
    std::string fname = "B02DstMuNu";
    ...
    for ( i = 0; i < nEvents; i++ ) {
        ...
        ev.p_mu.push_back(v_tl_D[1].Px());
        ev.p_mu.push_back(v_tl_D[1].Py());
        ev.p_mu.push_back(v_tl_D[1].Pz());
        ev.p_mu.push_back(v_tl_D[1].E());
        ev.p_mu.push_back(v_tl_D[1].M());

        ev.mu_PE = v_tl_D[1].E();
        ev.mu_PX = v_tl_D[1].Px();
        ev.mu_PY = v_tl_D[1].Py();
        ev.mu_PZ = v_tl_D[1].Pz();

        ev.mu_ID = parent->getDaug(1)->getPDGId();
...
void DumpResults(std::vector <event> v_evt) {
    ...
    TFile f("../root/B02DstMuNu.root","RECREATE");    
    ...
    t_evts->Branch("mu_PE", &ev.mu_PE);
    t_evts->Branch("mu_PX", &ev.mu_PX);
    t_evts->Branch("mu_PY", &ev.mu_PY);
    t_evts->Branch("mu_PZ", &ev.mu_PZ);
